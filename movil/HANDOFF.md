...
| **⭐ Demos** | Más → **Demos**: vitrina try-before-buy que se abren a pantalla completa (botón Abrir + Compartir por share nativo). **Demos del Motor 2 (Lector IA), standalone interactivas, marca Tríada:** **Auditor de Imagen** (autodiagnóstico → informe con puntaje), **Gemelo Virtual** (simulador de decisiones con sliders) y **Detección de Fugas** (montos editables → informe). **Plantillas de producto:** **CRM de Restaurantes** (4 roles) y **Barbería Triada** (web·reservas·cursos·CRM). Todas vendorizadas en `../demos/` y servidas por GitHub Pages. La lista se controla en `js/screens/demos.js` (`DEMOS`). Verificado: plantillas 2026-06-25; demos Motor 2 por render (Auditor 2026-07-01: cero overflow 375px, cero errores de consola) |
