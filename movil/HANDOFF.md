...
- **Compartir el PDF de la cotización:** `wa.me`/`mailto` NO adjuntan archivos → el único camino web para "enviar el PDF" es `navigator.share({files})` (share nativo del teléfono). El PDF-archivo se genera con **html2canvas + jsPDF** (CDN, `import()` dinámico — no hay CSP en el móvil) desde una hoja A4 **auto-contenida** (tokens `--rep-*` inline + ancho 794px), porque la `.report-page` de informe.css se **reflowea** en móvil (breakpoint <880px) y no sirve para capturar A4. Paginación con clamp: si `imgH ≤ 297mm+2` va a 1 hoja (evita una 2ª página vacía por redondeo sub-pixel). El "Descargar PDF" (window.print → @media print) es vector y separado del share.
