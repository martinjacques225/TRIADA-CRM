...
function prospectoBtn() {
  if (_lead) {
    return `<span style="width:36px;height:36px;border-radius:10px;background:var(--navy-l);color:var(--navy);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex:none">${e(initials(_lead.nombre))}</span>
      <span style="flex:1;min-width:0"><span class="ell" style="display:block;font-weight:600;font-size:15px;color:var(--ink)">${e(_lead.nombre)}</span><span class="ell" style="display:block;font-size:12px;color:var(--text2)">${e(_lead.empresa || '')}${_lead.correlativo ? ' · ' + e(_lead.correlativo) : ''}</span></span>`;
  }
  return `<span style="width:36px;height:36px;border-radius:10px;background:var(--surface2);color:var(--text3);display:flex;align-items:center;justify-content:center;flex:none">${ic('users', { size: 18 })}</span><span style="flex:1;color:var(--text2);font-size:15px">Vincular prospecto (opcional)</span>`;
}
...
export default {
  ...
  async render(app) {
    ...
    const tipoChip = (t) => `<button type="button" class="ct-chip" data-g="tipo" data-v="${t.id}" style="flex:none;display:flex;align-items:center;gap:6px;border:1px solid ${_form.tipo === t.id ? t.color : 'var(--border)'};background:${_form.tipo === t.id ? t.color + '18' : 'var(--surface)'};color:${_form.tipo === t.id ? t.color : 'var(--text2)'};border-radius:20px;padding:7px 12px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap"><span style="width:8px;height:8px;border-radius:50%;background:${t.color}"></span>${e(t.label)}</button>`;
    const plainChip = (g, v, label) => `<button type="button" class="ct-chip" data-g="${g}" data-v="${e(String(v))}" style="flex:none;border:1px solid ${_form[g] === v ? 'var(--teal)' : 'var(--border)'};background:${_form[g] === v ? 'var(--teal-l)' : 'var(--surface)'};color:${_form[g] === v ? 'var(--teal-d)' : 'var(--text2)'};border-radius:20px;padding:8px 14px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;font-variant-numeric:tabular-nums">${e(label)}</button>`;
    ...
    const memberAv = (m, i) => { const on = _form.participantes.includes(m.id); return `<button type="button" class="ct-part" data-id="${e(m.id)}" title="${e(m.nombre)}" style="width:38px;height:38px;border-radius:50%;background:${on ? memberColor(i) : 'var(--surface2)'};color:${on ? '#fff' : 'var(--text3)'};display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;border:2px solid ${on ? 'var(--surface)' : 'var(--border)'};cursor:pointer;${on ? '' : 'opacity:.7'}">${e(initials(m.nombre))}</button>`; };
      ...
      <div class="pad-form">
        <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;margin-bottom:16px;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm)">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:var(--text3)">${_form.id ? 'Creada por' : 'Organiza esta reunión'}</div>
            <div class="ell" style="font-size:13px;font-weight:600;color:var(--ink)">${e(creator.nombre)}${creatorIsMe ? ' · tú' : ''}</div>
          </div>
          <span style="font-size:11px;font-weight:600;color:var(--teal-d);background:var(--teal-l);padding:5px 9px;border-radius:20px;flex:none">Compartida</span>
        </div>
