    ...
    <div class="card" data-item="${i}" style="padding:14px">
      ...
      <div style="display:flex;align-items:center;gap:10px">
        <div style="display:flex;align-items:center;border:1px solid var(--border);border-radius:var(--radius-sm);overflow:hidden;flex:none">
          <button class="pp-dec" data-i="${i}" style="width:34px;height:38px;border:0;background:var(--surface2);color:var(--text);cursor:pointer;font-size:17px;line-height:1">−</button>
          <span class="tabular" data-qty="${i}" style="min-width:32px;text-align:center;font-weight:700;font-size:15px;color:var(--ink)">${Number(it.cantidad) || 1}</span>
          <button class="pp-inc" data-i="${i}" style="width:34px;height:38px;border:0;background:var(--surface2);color:var(--text);cursor:pointer;font-size:17px;line-height:1">+</button>
        </div>
      ...
      <div style="text-align:right;margin-top:9px;font-size:13px;color:var(--text2)">= <span class="serif tabular" data-line="${i}" style="font-size:17px;font-weight:600;color:var(--ink)">${formatCLP(lineOf(it))}</span></div>
    </div>`).join('');
}
...
export default {
  ...
  async render(app) {
    ...
    const estadoChip = (o) => `<button class="pp-estado" data-v="${o.v}" style="flex:none;border:1px solid ${_form.estado === o.v ? 'var(--teal)' : 'var(--border)'};background:${_form.estado === o.v ? 'var(--teal-l)' : 'var(--surface)'};color:${_form.estado === o.v ? 'var(--teal-d)' : 'var(--text2)'};border-radius:20px;padding:8px 14px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap">${e(o.label)}</button>`;
    const lead = _lead;
    ...
    <section class="screen" style="display:flex;flex-direction:column">
      ...
      <div class="pad-form">
        <button type="button" id="ppProspecto" style="width:100%;display:flex;align-items:center;gap:11px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:12px 13px;cursor:pointer;margin-bottom:18px;text-align:left;box-shadow:var(--shadow-sm)">
          ...
          <span style="flex:1;min-width:0"><span style="display:block;font-size:11px;color:var(--text3)">Para</span><span class="ell" style="display:block;font-weight:600;font-size:15px;color:var(--ink)">${lead ? e(lead.empresa || lead.nombre) : 'Vincular prospecto'}</span></span>
          ${ic('next', { size: 18, sw: 1.9 })}
        </button>

        <h2 class="serif" style="font-size:17px;font-weight:600;color:var(--ink);margin-bottom:10px">Ítems de servicio</h2>
        ...
        <div class="card" style="padding:16px;margin-top:16px">
          <div style="display:flex;justify-content:space-between;margin-bottom:9px"><span style="font-size:13px;color:var(--text2)">Subtotal</span><span class="tabular" id="propSub" style="font-size:15px;font-weight:600;color:var(--text)">${formatCLP(sub)}</span></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:11px;padding-bottom:11px;border-bottom:1px solid var(--border)"><span style="font-size:13px;color:var(--text2)">IVA 19%</span><span class="tabular" id="propIva" style="font-size:15px;font-weight:600;color:var(--text)">${formatCLP(iva)}</span></div>
          <div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:15px;font-weight:700;color:var(--ink)">Total</span><span class="serif tabular" id="propTot" style="font-size:24px;font-weight:600;color:var(--teal)">${formatCLP(tot)}</span></div>
        </div>
...
async function openProspectoPicker(app) {
  ...
  const rows = leads.map((l) => `<button class="pk-lead" data-id="${e(l.id)}" style="width:100%;display:flex;align-items:center;gap:11px;background:none;border:0;border-bottom:1px solid var(--border);padding:12px 2px;cursor:pointer;text-align:left"><span style="width:34px;height:34px;border-radius:10px;background:var(--surface2);color:var(--text2);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex:none">${e(initials(l.nombre))}</span><span style="flex:1;min-width:0"><span class="ell" style="display:block;font-weight:600;font-size:15px;color:var(--ink)">${e(l.nombre)}</span><span class="ell" style="display:block;font-size:12px;color:var(--text2)">${e(l.empresa || '')}</span></span></button>`).join('');
