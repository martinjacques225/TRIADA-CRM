...
function leadRow(l, st) {
  ...
  return `<div class="card--tap" data-lead="${e(l.id)}" style="display:flex;align-items:center;gap:11px;background:var(--surface);border:1px solid var(--border);border-left:3px solid ${st.color};border-radius:var(--radius-sm);padding:10px 12px;box-shadow:var(--shadow-sm)">
    ...
    <div style="flex:1;min-width:0"><div class="ell" style="font-weight:600;font-size:13px;color:var(--ink)">${e(l.nombre)}</div><div class="ell" style="font-size:12px;color:var(--text2)">${e(l.empresa || '—')}</div></div>
  ...
  return PIPELINE_STAGES.map((st) => {
    ...
    return `<div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        ...
        <span style="font-weight:700;font-size:15px;color:var(--ink)">${e(st.id)}</span>
        <span class="tabular" style="font-size:12px;font-weight:700;color:${st.color};background:${st.bg};padding:2px 9px;border-radius:20px">${items.length}</span>
  ...
  async render() {
    ...
    <section class="screen">
      <header class="hdr" style="display:flex;align-items:flex-end;justify-content:space-between">
        ...
        <div style="text-align:right"><div class="serif tabular" style="font-size:24px;font-weight:600;color:var(--teal);line-height:1">${activos}</div><div style="font-size:11px;color:var(--text3)">activos</div></div>
      </header>
