...
function msgHtml(m, i) {
  if (m.role === 'user') return `<div style="align-self:flex-end;max-width:84%;background:var(--teal);color:#fff;border-radius:16px 16px 4px 16px;padding:10px 14px;font-size:13px;line-height:1.45">${e(m.text)}</div>`;
  return `<div style="align-self:flex-start;max-width:90%;background:var(--surface2);color:var(--text);border:1px solid var(--border);border-radius:16px 16px 16px 4px;padding:11px 14px;font-size:13px;line-height:1.5">${m.text}${m.cta ? `<div><button data-cta="${i}" style="margin-top:9px;display:inline-flex;align-items:center;gap:6px;background:var(--surface);border:1px solid var(--teal);color:var(--teal);border-radius:20px;padding:6px 13px;font-family:var(--font);font-size:13px;font-weight:700;cursor:pointer">${e(m.cta.label)} ${ic('next', { size: 14, sw: 2.2 })}</button></div>` : ''}</div>`;
}
...
export function openTria(app) {
  ...
  openSheet(`
    <div style="display:flex;align-items:center;gap:12px;padding:6px 20px 14px;border-bottom:1px solid var(--border)">
      ...
      <div style="flex:1"><div style="display:flex;align-items:center;gap:7px"><span class="serif" style="font-size:17px;font-weight:600;color:var(--ink)">trIA</span><span class="tria-ia">ASISTENTE IA</span></div><div style="font-size:12px;color:var(--text2)">Tu copiloto de terreno</div></div>
    ...
    <div style="padding:0 20px 8px;display:flex;gap:9px;align-items:center">
      <div style="flex:1;display:flex;align-items:center;gap:8px;background:var(--surface2);border:1px solid var(--border);border-radius:24px;padding:0 6px 0 15px;height:46px">
        <input id="tria-input" placeholder="Escríbele a trIA…" style="border:0;background:none;outline:none;flex:1;font-family:var(--font);font-size:15px;color:var(--text)">
