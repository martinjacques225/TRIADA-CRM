...
export async function openCampana(app) {
  ...
  const items = upcoming.map(({ c, cd }) => {
    ...
    return `<button class="cmp-item" data-go="agenda" style="width:100%;display:flex;align-items:center;gap:12px;background:var(--surface);border:1px solid var(--border);border-left:3px solid ${t.color};border-radius:var(--radius-sm);padding:12px 13px;margin-bottom:9px;cursor:pointer;text-align:left">
      <div style="flex:1;min-width:0"><div class="ell" style="font-weight:700;font-size:13px;color:var(--ink)">${e(c.titulo || t.label)}</div><div class="ell" style="font-size:12px;color:var(--text2);margin-top:2px">${emp ? e(emp) + ' · ' : ''}${e(c.hora || '')} · ${e(t.label)}</div></div>
      <span style="font-size:12px;font-weight:700;color:${cd.soon ? 'var(--danger)' : t.color};background:${cd.soon ? 'var(--danger-l)' : t.color + '14'};padding:4px 10px;border-radius:20px;white-space:nowrap;flex:none">${e(cd.label)}</span>
    </button>`;
  }).join('');
  ...
  const inviteItems = invites.map((c) => {
    ...
    return `<button class="cmp-item" data-go="agenda" style="width:100%;display:flex;align-items:center;gap:12px;background:var(--surface);border:1px solid var(--border);border-left:3px solid ${t.color};border-radius:var(--radius-sm);padding:12px 13px;margin-bottom:9px;cursor:pointer;text-align:left">
      ...
      <div style="flex:1;min-width:0"><div class="ell" style="font-weight:700;font-size:13px;color:var(--ink)">${e(c.titulo || t.label)}</div><div class="ell" style="font-size:12px;color:var(--text2);margin-top:2px">Te sumaron a esta reunión${emp ? ' · ' + e(emp) : ''}${c.fecha ? ' · ' + e(c.fecha) : ''}</div></div>
    </button>`;
  }).join('');
  ...
  const nudge = porContactar ? `<button class="cmp-item" data-go="leads" style="width:100%;display:flex;align-items:center;gap:12px;background:var(--teal-l);border:1px solid var(--border);border-radius:var(--radius-sm);padding:12px 13px;margin-bottom:9px;cursor:pointer;text-align:left"><span style="width:34px;height:34px;border-radius:10px;background:var(--surface);color:var(--teal);display:flex;align-items:center;justify-content:center;flex:none">${ic('users', { size: 18 })}</span><div style="flex:1"><div style="font-weight:700;font-size:13px;color:var(--ink)">${porContactar} ${porContactar === 1 ? 'lead' : 'leads'} por contactar</div><div style="font-size:12px;color:var(--text2)">No los dejes enfriar</div></div></button>` : '';
