...
- **⬜ Ojo:** `informe.paginate.js` era **archivo nuevo** en el commit anterior (se bajó fresco) pero `informe.css` ya existía y GitHub Pages lo sirve con `Cache-Control: max-age=600` — un navegador con el CSS viejo en caché y el JS nuevo da exactamente el mismo síntoma. Tras desplegar, **hard-refresh (Ctrl+Shift+R)** antes de dar nada por bueno. Sigue sin haber versionado (`?v=`) en los `<link>` de `index.html`: candidato a arreglar.
