...
- **⚠️ El CÓDIGO del frontend NO está desplegado:** la DB está lista, pero el módulo vive solo en local (sin commit/push). En el CRM en vivo (GitHub Pages) Contratos aparecerá recién tras desplegar. Al desplegar, ojo con el gotcha de Pages (re-run del job + verificar `Last-Modified`).
- **🟡 Sin verificar (requiere el CRM logueado):** el round-trip real de guardar/emitir end-to-end desde la UI (asignación de correlativo con sesión, subida del master, aislamiento RLS cross-tenant en vivo) y el click-through completo.
