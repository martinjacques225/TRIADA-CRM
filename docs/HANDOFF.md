...
- **Fase 4 (contrato Impulsa) HECHA:** se redactó el canónico `05-VENTAS/_fuente/contrato-impulsa-adhesion.html` (Contrato de Adhesión al Programa Impulsa Pyme — 25 fills; precio FIJO $300.000 neto/$357.000 IVA incl., 6 meses; perfeccionamiento contrato+primer pago §5, límites de contabilidad §9, dominio/fin de programa §10/§14, uso en portafolio, obligación de medios). Sumado a `render-jobs.txt`, `vendor-contratos.py` (FILL_KEYS, 10/10) y `schemas.js` (id `impulsa`, prefijo TRD-IMP-, precio fijo → `money:null`). Verificado: `node --check`, editor con 25 fills + precio fijo en preview, **PDF real por render.py** correcto. ⚠️ Plantilla BASE: requiere ratificación de abogado/a antes de usar con clientes (como todas).
- **⬜ Biblioteca (parte de F4, PENDIENTE — requiere al dueño):** subir a biblioteca el material de campaña catalogado (dossier Impulsa, Bases, protocolo de ventas, guion, precios, cumplimiento, decks→PDF). Son **subidas de archivos** que se hacen desde el módulo Biblioteca (arrastrar) con sesión iniciada; no se pueden automatizar sin login. Checklist/Matriz de Evaluación = USO INTERNO (categoría aparte). Decks HTML: renderizar a PDF con render.py antes de subir.
