/* ============================================================
   BARBERÍA TRIADA · <image-slot>
   Placeholder de imagen que el dueño rellena arrastrando una foto
   (o haciendo clic para buscar). La foto persiste en localStorage,
   así que sobrevive recargas y queda guardada en el navegador del
   cliente que prueba la demo. Adaptado del componente de Claude
   Design: se cambió la persistencia (sidecar → localStorage) y se
   quitó el gate de runtime, así funciona abriendo el index.html
   directo o servido en Vercel.

   Atributos:
     id           clave de persistencia (REQUERIDA para que sobreviva)
     shape        rect | rounded | circle | pill        (def. rounded)
     radius       radio en px para 'rounded'            (def. 12)
     fit          object-fit: cover | contain | fill    (def. cover)
                  con cover, doble-clic entra a modo reencuadre.
     placeholder  texto del estado vacío
   ============================================================ */
(() => {
  const LS_KEY = 'bt.imageslots.v1';
  const MAX_DIM = 1200;
  const ACCEPT = ['image/png', 'image/jpeg', 'image/webp', 'image/avif'];
  const ACCENT = '#0C7C88'; // teal Tríada (coincide con --color-primary)

  // ── Store compartido (un solo objeto en localStorage) ──────────────────
  const subs = new Set();
  let slots = {};
  try { slots = JSON.parse(localStorage.getItem(LS_KEY) || '{}') || {}; } catch { slots = {}; }
  ...
  const stylesheet =
    ...
    '  background:#fff;box-shadow:0 0 0 1.5px ' + ACCENT + ',0 1px 3px rgba(0,0,0,.3);' +
    ...
    ':host([data-reframe]) .frame{box-shadow:0 0 0 2px ' + ACCENT + '}' +
    ...
    '  white-space:nowrap}' +
    ':host([data-filled]:hover) .ctl,:host([data-reframe]) .ctl{opacity:1;pointer-events:auto}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '</div>' +
        '<div class="ctl"><button data-act="replace">Cambiar</button>' +
        '  <button data-act="clear">Quitar</button></div>' +
      ...
      root.addEventListener('click', (e) => {
        ...
        if (act === 'clear') {
          this._exitReframe(false); this._gen++; this._local = null;
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Arrastra una imagen PNG, JPEG, WebP o AVIF.');
        return;
      }
      const gen = ++this._gen;
      try {
        ...
        if (gen !== this._gen) return;
        this._setError('No se pudo leer esa imagen.');
      }
    }
    _setError(msg) {
    ...
    _render() {
      ...
      this._cap.textContent = this.getAttribute('placeholder') || 'Arrastra una imagen';
      if (url) {
        ...
        this._img.removeAttribute('src'); this._ghost.removeAttribute('src');
